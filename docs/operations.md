...
`vector-ui-grid-kit` is centered on frontend apps work in Lua. The technical target is: Develop a Lua command-oriented project for grid scenarios with bounded scenario files, conflict explanations, and fixture-scale datasets.
...
The score is deterministic. The current threshold is 163. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 4.
